...
def metrics(data: pd.DataFrame):
    ...
    global VENDOR_ID
    global VENDOR_NAME
    global VENDOR_VERSION

    if os.path.isfile('./MetadataInformation.txt'):
        logger.info("found MetadataInformation.txt")
        with open('./MetadataInformation.txt') as file:
            file_content = file.read()
            logger.info("Reading MetadataInformation.txt")
        VENDOR_ID, VENDOR_NAME, VENDOR_VERSION = extract_metadata(file_content)
    else:
...
# Extracts metadata from the content of a text file.
# Aims to retrieve VendorID, VendorName, VendorVersion.
#
# Parameters:
#     file_content (str): String content of a text file
# Returns:
#     metadata (tuple): Tuple holding the metadata VendorID, VendorName, VendorVersion
#     in that order.
def extract_metadata(file_content: str):
    pattern = re.compile(r"Metadata:\n.*: (.*)\n.*: (.*)\n.*: (.*)\n")
    regex = re.search(pattern, file_content)
    if regex:
